...
pub async fn argument(context: &Context, message: &Message, mut args: Args) -> CommandResult {
    if message.mentions.len() != 1 {
        message
            .channel_id
            .say(&context.http, MISSING_DISSENTER)
            .await?;
        return Ok(());
    }
    ...
    let dissenter: &User = message
        .mentions
        .get(0)
        .expect("Should have guaranteed mention");
    // TODO: https://docs.rs/serenity/latest/serenity/utils/fn.parse_username.html
    let _discard = args
        .single::<String>()
        .expect("Should have dissenter mentioned");
    ...
    match created_argument {
        ...
        Err(_e) => {
            message
                .channel_id
                .say(&context.http, "Failed to create argument")
                .await?;
            Ok(())
        }
    }
...
pub async fn list_arguments(context: &Context, message: &Message, _args: Args) -> CommandResult {
    ...
    match arguments_result {
        Ok(arguments) => {
            ...
            message
                .channel_id
                .say(&context.http, arguments_message)
                .await?;
            Ok(())
        }
        Err(_e) => {
            message
                .channel_id
                .say(&context.http, "Failed to parse arguments")
                .await?;
            Ok(())
        }
    }
...
pub async fn take_your_notch(
    ...
    match argument_result {
        Err(_e) => {
            message
                .channel_id
                .say(&context.http, FAILED_TO_GET_ARGUMENT)
                .await?;
            Ok(())
        }
        Ok(argument) => {
            ...
            if argument_taken {
                message
                    .channel_id
                    .say(&context.http, NOTCH_ALREADY_TAKEN)
                    .await?;
                Ok(())
            } else {
                ...
                match notch_taker_id {
                    ...
                    None => {
                        message
                            .channel_id
                            .say(&context.http, NOT_PART_OF_ARGUMENT)
                            .await?;
                        Ok(())
                    }
                }
...
pub async fn leaderboard(context: &Context, message: &Message, _args: Args) -> CommandResult {
    ...
    match taken_arguments {
        Ok(arguments) => {
            let stats: Vec<(i64, usize)> = arguments
                .into_iter()
        ...
        Err(_e) => {
            message
                .channel_id
                .say(&context.http, "Failed to get taken arguments")
                .await?;
            Ok(())
        }
    }
